...
版本说明（v3.2）：
    - 学生端：填写 姓名 / 学号 / 联系方式 + PHQ-9 + GAD-7 + 心情自述
    - 学生端提交后：只显示"提交成功"，不显示任何得分与风险等级
    - 管理员端：需密码进入，可查看全部记录（含身份信息）
...
# =========================================================
# 管理员密码管理（持久化到 admin_config.json）
# =========================================================
def _initial_password() -> str:
    """初始密码优先级：Streamlit Secrets > 默认值"""
    try:
...
def page_home():
    ...
    st.info(
        ...
        "🔐 **管理员端**：需密码登录，可查看全部预警结果、含姓名/学号/联系方式的完整记录与干预建议。"
    )
...
def page_assess():
    ...
    with st.form("assess_form", clear_on_submit=False):
        ...
        with col_c:
            contact = st.text_input("联系方式 *", placeholder="手机号 / 微信 / QQ")
    ...
    st.markdown(
        ...
        <div style="background-color:{bg}; color:{fg};
            ...
            <p style="color:{fg}; margin-top:6px;">
                姓名：{latest.get('姓名','')}　|　学号：{latest.get('学号','')}　|　
                联系方式：{latest.get('联系方式','')}
            </p>
...
def main():
    ...
    init_data_file()
    # 初始化配置文件（确保首次运行就有 admin_config.json）
    load_admin_password()
