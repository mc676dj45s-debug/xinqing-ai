...
"""
大学生心理健康预警系统
文件命名：xinqing-ai2.0.py
参考论文：《基于人工智能技术的大学生心理健康预警系统设计》黄丽芬

改造说明（权限分离版）：
    - 学生端：只开放「心理测评」页面，提交后不显示任何得分与风险结果。
    - 管理员端：需密码进入，可查看「系统首页」+「风险预警与干预」。
    - 六大模块仍完整保留，只是把「预警/干预」的展示权限交给管理员。
"""
...
def page_home():
    ...
    st.info(
        ...
        "🔐 **管理员端**：需密码登录，可查看全部预警结果、干预建议与历史记录。"
    )
...
def page_assess():
    ...
    # 清除上次填写的缓存（可选）
    # 如果希望提交后表单清空，可把上面的 clear_on_submit 改为 True
...
def page_warning():
    ...
    st.markdown(
        ...
        <div style="background-color:{bg}; color:{fg};
            ...
            <p style="color:{fg}; margin:0;">{ALERT_MSG.get(level, '')}</p>
        </div>
        """,
    ...
    if not high_df.empty:
        st.markdown("### 🚨 高风险学生名单")
        high_show = high_df[["提交时间", "PHQ9总分", "GAD7总分", "风险总分", "心情自述"]].copy()
    ...
    show_cols = ["提交时间", "PHQ9总分", "GAD7总分", "关键词得分", "风险总分", "风险等级"]
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.caption("🔒 学生填写内容仅管理员可见")
        st.caption("📁 数据保存在本地 CSV")

    # ---------- 路由 ----------
    if role == "🎒 学生（填写测评）":
        # 学生端：只显示心理测评页，无导航、无结果
        page_assess()
        return
